...
import { parseAttachments, sortAttachments } from '@/lib/types/attachments';
...
// Project type based on Prisma schema
...
export function ProjectDetailClient({ slug, initialProject }: ProjectDetailClientProps) {
  const locale = useLocale() as 'it' | 'en' | 'fr';
  const t = useTranslations('projects');

  // Use initial project data directly (already fetched server-side)
  ...
  // Fetch all projects for navigation (client-side, non-critical)
  ...
  const galleryImages = React.useMemo(
    () => parseGallery(project.gallery),
    [project]
  );
  const lightboxImages: LightboxImage[] = React.useMemo(
    () => galleryToLightboxImages(galleryImages),
    [galleryImages]
  );

  // Parse project attachments
  const projectAttachments = React.useMemo(
    () => sortAttachments(parseAttachments(project.attachments)),
    [project]
  );

  const openLightbox = React.useCallback((index: number = 0) => {
  ...
  // Get translation (server already filtered it)
        ...
        <div className="max-w-lg w-full text-center">
          <div className="mb-8">
            ...
            <p className="text-slate-600">
              La traduzione per questo progetto non è disponibile.
            </p>
          </div>
          <Link
            href="/progetti"
                    ...
                    {galleryImages.map((image, index) => (
                      ...
                      >
                        <Image
                          ...
                          alt={
                            image.title ||
                            `${translation.title} - Immagine ${index + 1}`
                          }
                          fill
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          >
            ...
            <p className="text-xl text-white/80 mb-8 max-w-2xl mx-auto">
              Contattaci per una consulenza gratuita. Il nostro team ti aiuterà a
              trovare le migliori soluzioni per il tuo progetto.
            </p>
